...
import { BODHI_MENU_CATALOG } from '@/data/bodhiMenuSeed'
import { generateMockOrders } from '@/data/mockAnalytics'
import type {
  MenuItem,
  OrderItemRecord,
  OrderRecord,
...
const ORDERS_TABLE = 'bodhi_orders'
const ORDER_ITEMS_TABLE = 'bodhi_order_items'
...
export async function fetchOrdersWithItems(): Promise<OrderWithItems[]> {
  try {
    ...
    assertNonEmpty(menu, MENU_TABLE, 'supabase/migrations/001_bodhi_full.sql')
    assertNonEmpty(orders, ORDERS_TABLE, 'supabase/migrations/001_bodhi_full.sql')
...
async function insertDemoOrders(): Promise<void> {
  ...
  for (const order of mockOrders) {
    ...
    const rows = order.items
      .filter((item) => BODHI_MENU_CATALOG[item.menu_item_id])
      .map((item) => ({

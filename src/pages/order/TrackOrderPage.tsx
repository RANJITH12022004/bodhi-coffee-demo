    ...
    <main className="min-h-screen px-margin-mobile pb-margin-mobile pt-md">
            ...
            return (
              <li key={step.key} className="flex items-start gap-md">
                ...
                <div>
                  ...
                  {active && order.status === 'completed' && (
                    <p className="text-sm text-on-surface-variant">Enjoy your coffee!</p>
                  )}
                </div>
              </li>
            )
      ...
      {bill && bill.rounds.length > 0 && (
        <section className="mt-lg rounded-xl border border-outline-variant/50 bg-surface-container-low p-md">
          ...
          {(PRICES_INCLUDE_GST || GST_RATE === 0) && (
            <p className="mt-1 text-xs font-semibold text-primary">Prices include GST</p>
          )}
          ...
          <p className="mt-sm text-xs text-on-surface-variant">
            Multiple orders from this table are combined into one bill.
            {RESTAURANT_CHARGE > 0 ? ' Restaurant charge applies once.' : ''}
          </p>
        </section>
      )}

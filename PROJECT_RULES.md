...
This is a **sales demo**, not a production SaaS. It exists to show café and restaurant owners in Bangalore/Hyderabad a live, working flow in a 5-minute pitch: customer orders via QR → kitchen sees it instantly → manager tracks stock → owner sees the numbers. It will be demoed repeatedly to different leads, not deployed to a real paying restaurant on day one.
...
- **Styling:** Tailwind CSS with Bodhi earthy palette (cream base #F7F3EC, primary #2F4F3E, accent brown #3D2B1F; KDS screens use dark charcoal theme)
...
- **No multi-restaurant tenancy.** Hardcode "Bodhi Coffee" as the one café. If asked to support multiple restaurants later, that's a different project.
...
Seed all tables with realistic fake data matching the Bodhi Coffee menu (café categories: classics, signatures, chilled, food, addons, etc.). **A demo with empty tables is a dead demo** — never leave this to "add data later."
...
Build a PIN-based staff login screen at route /staff for the Bodhi Coffee café demo app.
...
Build the Stock Manager dashboard at route /stock for the Bodhi Coffee café demo app, matching the Stitch designs already generated for this screen.
...
Build the Owner analytics dashboard at route /owner for the Bodhi Coffee café demo app, matching the Stitch designs already generated for this screen.
